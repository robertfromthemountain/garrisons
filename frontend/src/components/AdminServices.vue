    ...
    <v-table height="100vh" fixed-header class="bg-garrisons">
      ...
      <tbody>
        ...
        <tr v-for="(service, index) in filteredServices" :key="service.id">
          ...
          <td v-else>
            <input v-model="editServiceData.price" type="number" />
          </td>

          <td v-if="!isEditing[index]">{{ service.duration }}</td>
          <td v-else>
            <input v-model="editServiceData.duration" type="number" />
          </td>
...
export default {
  setup() {
    ...
    return {
      t,
      services,
      searchQuery, // Return search query for services
      filteredServices, // Return filtered services based on search query
      isEditing,
      editServiceData,
      newService,
      startEdit,
